...
Renderer::Renderer() {
    ...
    viewer.core().background_color << 0.5f, 0.78f, 0.89f, 1.f; // set background to blue

    viewer.launch_init();
    // Initialize the rendering buffers VAO, VBO, EBO and stuff...
    viewer.data().meshgl.init();

    // @TODO: Don't use relative paths like this. Maybe there is a clean c++ variant?
    // Load the static shader source and compile it
...
void Renderer::registerStatics(std::vector<Mesh> &list) {
    // This is separated if we want to extend the difference between static and dynamics
    for (Mesh &mesh : list) {
...
void Renderer::setMeshData(std::vector<Mesh> &list) {
    for (Mesh &mesh : list) {
        ...
        // Update renderable, note that both getters return a copy and not a reference.
