 ...
 * @brief Defines the SimulationModel class which is responsible for the physical
 * simulation itself. It also provides an interface to access the data.
 */
#pragma once
#include <Eigen/Core>
#include "Mesh.h"
 ...
 * @brief The physical model class of XPBD. Provides an interface to the current state
 * of the meshes, as well as an interface to update the physical model for some time.
 * 
 * The data is split up into two parts:
 * - Static Data:  This data is static and should not move over time. An example for
 *                 this is the floor. In the future, it should also be the colliders
 * - Dynamic Data: This data is the data which should be updates each time step and
 *                 should behave just like XPBD definition according to the constraints.
 */
class SimulationModel {
public:
    /**
...
private:
    /**
     * @brief Initialize the basic rendering scene with static and dynamic objects.
     */
    void initialize();
