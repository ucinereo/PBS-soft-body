...
/**
 * @class Mesh
 * @brief Mesh representation class with vertices, faces and colors. Furthermore it
 * stores an ID per mesh which is the index the mesh has in the renderer data structure.
 */
class Mesh {
public:
    /**
     ...
     * @param F Faces matrix of shape (N x 3), note that the sequence per triangle is
     * important, as (1, 2, 3) and (3, 2, 1) are the same triangle, but with inverted
     * normals!
     */
     ...
     * @param V New vertex matrix of shape (N, 3), note that this function does not
     * check wether the new vertex positions are valid.
     */
...
private:
    // @TODO: Currently stores the vertex positions directly, might be smart to add an
    // origin position just like in the PBS example projects.
    ...
    Eigen::Vector4f color = Eigen::Vector4f(0.2f, 0.2f, 0.2f, 1.0f); ///< Mesh color
